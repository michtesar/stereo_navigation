...
try
    ...
    drawsphere(-1.5, 0, 2, [1, 0, 0, 0]);
    drawsphere(2.5, 0, -1, [1, 1, 1, 0]);
    drawfloor();
    drawarena();
    glPopMatrix;
catch
...
function drawarena()
global GL
readtexture('paper.jpg');
glMaterialfv(GL.FRONT, GL.DIFFUSE, [1, 1, 1, 0]);
qobj = gluNewQuadric();
gluQuadricTexture(qobj, GL.TRUE);
glRotatef(90, 1, 0, 0);
glTranslatef(-1.15, 2, 0);
glEnable(GL.TEXTURE_2D);
gluCylinder(qobj, 8, 8, 10, 30, 30);
glDisable(GL.TEXTURE_2D);
end

function drawfloor()
global GL
readtexture('fabric.jpg');
